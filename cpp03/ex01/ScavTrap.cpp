...
void ScavTrap::attack(const std::string& target)
{
    if (_hitPoints == 0 || _energyPoints == 0){
        std::cout << "Scavtrap " << _name << " is dead or exhausted, he can't do nothing" << std::endl;
    }
    else{
...
ScavTrap::ScavTrap(): ClapTrap()
{
    ...
    _energyPoints = 50;
    _attackDammage = 20;
    std::cout << CYN << "ScavTrap default constructor for: " << _name << RESET << std::endl;
}
...
ScavTrap::ScavTrap(const ScavTrap & rhs): ClapTrap(rhs.getName())
{
    ...
    std::cout << CYN << "ScavTrap constructor copy called for " << _name << RESET << std::endl;
}
...
ScavTrap::ScavTrap(const std::string name): ClapTrap(name)
{
    ...
    _energyPoints = 50;
    _attackDammage = 20;
    std::cout << CYN << "ScavTrap constructor for: " << name << RESET << std::endl;
}
